...
class SpatialProximityMapper:
    ...
    def map_image_to_context(self, image_bbox, document_elements) -> Dict:
        ...
        return {
            ...
            # Multi-context assembly (your innovation!)
    ...
    def _find_primary_heading(self, img_bbox, elements) -> Optional[Dict]:
        """Find the most relevant heading using spatial intelligence"""
        headings = [e for e in elements if e.get('type') == 'heading']
        
        if not headings:
            return None
            
        # Calculate spatial distances with smart weighting
        scored_headings = []
        for heading in headings:
            # Vertical distance (most important for document flow)
            y_distance = abs(img_bbox.y0 - heading['bbox'][3])  # Top of image to bottom of heading
            
            # Horizontal alignment bonus
            x_overlap = max(0, min(img_bbox.x1, heading['bbox'][2]) - max(img_bbox.x0, heading['bbox']))
            alignment_bonus = x_overlap / max(img_bbox.width, heading['bbox'][2] - heading['bbox'])
            
            # Heading level bonus (H1 > H2 > H3)
            level_bonus = max(0, 4 - heading.get('level', 3)) * 10
            
            # Combined score (lower is better, but add bonuses)
            score = y_distance - (alignment_bonus * 50) - level_bonus
            
            scored_headings.append((heading, score))
        
        # Return best match
        best_heading = min(scored_headings, key=lambda x: x[1])
        return best_heading
